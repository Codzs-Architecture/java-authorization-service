...
import com.codzs.handler.oauth2.authentication.federation.FederatedIdentityAuthenticationSuccessHandler;
...
public class DefaultSecurityConfig {
	...
	public SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
		http
			.securityMatcher(OAuth2Constant.Resources.ASSETS, OAuth2Constant.Endpoints.LOGIN)
			.authorizeHttpRequests(authorize ->
				authorize
					.requestMatchers(OAuth2Constant.Resources.ASSETS, OAuth2Constant.Endpoints.LOGIN).permitAll()
					.anyRequest().authenticated()
			)
			.formLogin(formLogin ->
				formLogin
					.loginPage(OAuth2Constant.Endpoints.LOGIN)
			)
	...
	public SecurityFilterChain managementSecurityFilterChain(HttpSecurity http) throws Exception {
		http
			.securityMatcher(OAuth2Constant.Management.MANAGEMENT, OAuth2Constant.Management.ACTUATOR)
			.authorizeHttpRequests(authz -> authz
				.requestMatchers(OAuth2Constant.Management.HEALTH, OAuth2Constant.Management.INFO).permitAll()  // Only health and info are public
				.requestMatchers(OAuth2Constant.Management.MANAGEMENT, OAuth2Constant.Management.ACTUATOR).authenticated()  // All other endpoints require authentication
				.anyRequest().authenticated()
			)
